...
const render = () => {
  ...
  const width = Math.max(container.value.clientWidth || 1000, props.notesData.length * 60)
  const height = 240
  ...
  const staveNotes = props.notesData.map((data) => {
    ...
    note.addModifier(factory.Fingering({ number: data.fingering, position: props.clef === 'treble' ? 3 : 4 }), 0) // Position values vary by version

    return note
  })

  const voice = factory.Voice({ time: { num_beats: props.notesData.length, beat_value: 4 } }).addTickables(staveNotes)

  const textNotes = props.notesData.map((data) => {
    return factory.TextNote({ text: data.label, duration: "q" })
      .setLine(12)
      .setJustification(2) // CENTER
  })

  const textVoice = factory.Voice({ time: { num_beats: props.notesData.length, beat_value: 4 } }).addTickables(textNotes)

  factory.Formatter().joinVoices([voice, textVoice]).format([voice, textVoice], width - 100)
  ...
  staveNotes.forEach((staveNote, idx) => {
    ...
    if (element) {
      ...
      element.addEventListener("click", () => {
        emit('play-note', { note: props.notesData[idx].key.replace("/", "").toUpperCase(), element })
      })
    }
  })
}
...
<template>
  <div class="section-header">
    <h2>{{ title }}</h2>
    <div class="line"></div>
    <div :class="['hand-badge', badgeClass]">{{ clef === 'treble' ? 'Treble Clef' : 'Bass Clef' }}</div>
  </div>

  <div class="staff-wrap">
    <div class="staff-card">
      <div class="staff-card-header">
        <h3>{{ clef === 'treble' ? 'Treble Clef' : 'Bass Clef' }} · {{ title }} · {{ range }}</h3>
        <button class="play-all-btn" @click="$emit('play-all', clef)">Play Scale</button>
      </div>
